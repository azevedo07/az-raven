...
/**
 * POST /api/pipeline/:projectId/resume
 *
 * HTTP → ResumeProjectUseCase → PipelineService → PipelineEngine → Registry
...
export async function POST(_request: Request, { params }: { params: { projectId: string } }) {
  return respondToAction(
    () => useCases.resumeProject.execute({ projectId: params.projectId }),
    "RESUME_PROJECT_FAILED"
  );
}
